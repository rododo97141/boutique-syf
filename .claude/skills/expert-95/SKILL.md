...
**Garde-fou d'auto-modification (autorisation obligatoire)** : en Claude Code, le skill suit sa propre progression au fil des usages — ce qui a bien ou mal fonctionné, ce qu'il a appris, les points encore à optimiser — et peut **proposer** de mettre à jour son propre fichier (`SKILL.md` ou `memoire.md`). Mais il ne modifie **JAMAIS** son fichier sans accord explicite de l'utilisateur. Avant toute mise à jour, il doit systématiquement : (1) **afficher le fichier concerné** (ou le diff exact proposé) ; (2) **lister clairement les points qui seront améliorés** et ceux qui restent à optimiser ; (3) **demander l'autorisation** — l'utilisateur répond oui ou non. Sur « oui » seulement, il applique la modification (édition + commit en Claude Code) et confirme ce qui a changé. Sur « non », il n'écrit rien et conserve la version actuelle. Le skill informe ainsi toujours **à l'avance** de ce qui va changer. (Note : seul Claude Code peut éditer réellement le fichier ; le chat Claude classique peut seulement proposer le texte. Ce qui progresse est la méthode écrite dans le fichier — sous le contrôle de l'utilisateur — non le modèle lui-même.)
