...
| **Nos Signatures** (la vitrine saveurs de l'accueil) | ~~Nos Cocktails Signature~~ | Amendement R12 : le titre de la section cartes pochettes de l'accueil (accent Fraunces sur *Signatures*). Ne renomme ni la page `saveurs.html`, ni la row du méga-menu (« Les pochettes SYFIR »). |
| **Collection Syf** (l'entrée de nav vers `saveurs.html`) | ~~Saveurs~~, ~~Notre Gamme~~ (en libellé) | Amendement R19 : le libellé de navigation est **Collection Syf** partout (nav pilule 7 pages, menu mobile, sidebar méga-menu, aria) ; en nav (capitales via CSS) s'affiche **COLLECTION SYF**. L'URL reste `saveurs.html` ; le hero de la page s'aligne (eyebrow « La collection », h1 « Collection *Syf* », title/og « Collection Syf »). |
| **Panneau Collection Syf** — 3 entrées catégories | ~~liste de produits~~ | Amendement R15/R19 : le déroulant de nav est un panneau pleine largeur (vignettes + libellés capitales) à trois catégories : **Toute la collection** (`saveurs.html`), **Nos cocktails** (`saveurs.html#cocktails`, la grille — expérience « sur place, dans le verre ») et **Nos signatures** (`saveurs.html#signatures`, la pochette signature — expérience « à emporter »). Pas de noms de produits individuels dans la nav. |
