...
| **Notre Collection** (l'entrée de nav vers `saveurs.html`) | ~~Saveurs~~ (en libellé) | Amendement R13 : le libellé de navigation devient **Notre Collection** partout (nav pilule, sidebar méga-menu, menu mobile, aria) ; en nav (capitales via CSS) s'affiche **NOTRE COLLECTION**. L'URL reste `saveurs.html` (le mot *saveurs* survit dans le chemin, la description et les `data-nav`, ce sont des identifiants, pas du libellé) ; le hero de la page s'aligne (« Notre *Collection* »). Le panneau déroulant garde son titre « Nos Signatures ». |
