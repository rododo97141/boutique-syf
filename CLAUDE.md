...
| **Notre Gamme** (l'entrée de nav vers `saveurs.html`) | ~~Saveurs~~ (en libellé) | Amendement R13 : le libellé de navigation est **Notre Gamme** partout (nav pilule, sidebar méga-menu, menu mobile, aria) ; en nav (capitales via CSS) s'affiche **NOTRE GAMME**. L'URL reste `saveurs.html`, le hero de la page s'aligne (« Notre *Gamme* »). |
| **Dropdown Notre Gamme** — 3 entrées catégories | ~~liste de produits~~ | Amendement R14 (décision finale) : le déroulant de nav est une **liste texte** (pas de cartes) à trois catégories : **Toute notre gamme** (`saveurs.html`), **Nos cocktails** (`saveurs.html#cocktails`, la grille — expérience « sur place, dans le verre ») et **Nos signatures** (`saveurs.html#signatures`, la pochette signature — expérience « à emporter »). Pas de noms de produits individuels dans la nav. |
