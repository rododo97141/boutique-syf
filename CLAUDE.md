...
| **Nos Signatures** (la vitrine saveurs de l'accueil + titre du panneau déroulant) | ~~Nos Cocktails Signature~~ | Amendement R12 : le titre de la section cartes pochettes de l'accueil et du panneau déroulant en cartes de la nav (accent Fraunces sur *Signatures*). Ne renomme ni la page `saveurs.html`, ni la row du méga-menu (« Les pochettes SYFIR »). |
| **Notre Gamme** (l'entrée de nav vers `saveurs.html`) | ~~Saveurs~~ (en libellé) | Amendement R13 : le libellé de navigation devient **Notre Gamme** partout (nav pilule, sidebar méga-menu, menu mobile, aria) ; en nav (capitales via CSS) s'affiche **NOTRE GAMME**. L'URL reste `saveurs.html` (le mot *saveurs* survit dans le chemin, la description et les `data-nav`, ce sont des identifiants, pas du libellé) ; le hero de la page s'aligne (« Notre *Gamme* »). Le panneau déroulant garde son titre « Nos Signatures ». |
