...
14. **L20 (recherche dans le code)** — 5 blocs, un commit chacun :
    ...
    **C. Créativité** : tilt 3D `data-tilt` (6° max, retour 180 ms, souris seule) sur les vitrines du sachet ; passe halo → 0 élément interactif sans hover (audit programmatique, .nav-logo/.scroll-hint corrigés).
...
15. **L19 (copy v2)** Réécriture éditoriale complète, textes rédigés par le superviseur : accueil (Trois dates. Zéro excuse. / teasers / intros sections / CTA final), descriptions sensorielles des 3 cocktails, actualité (intro Ambiance + newsletter « Je m'inscris »), espace pro billetterie, note de partage fiche événement (.ed-share-note). **Tutoiement généralisé** sur les 6 pages + tous les JS (formulaires, toasts, validations, placeholders) — seuls restent en vouvoiement : la porte d'âge et les mentions sanitaires/légales (intouchées, consigne). Slogan et manifesto inchangés.
